...
from dataloaders.eye.eyeDataset import UnityDataset
...
def main(args):
    ...
    for i_iter in range(args.total_iterations):
        ...
        if i_iter % args.iters_to_eval == 0:
            ...
            gen_target_img = torchvision.utils.make_grid(target_img, padding=2, normalize=True)
            torchvision.utils.save_image(gen_target_img, filename)

            filename = os.path.join(args.exp_dir, "Unity_pred_trainiter_{}.png".format(i_iter))
            pred_img = pred_img.float()
            gen_img = torchvision.utils.make_grid(pred_img, padding=2, normalize=True)
