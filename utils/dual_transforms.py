...
class ToTensor(object):
	def __call__(self, img, pos, t_type='pos'):
		if pos is None:
			return t_transforms.ToTensor()(img), None
		return t_transforms.ToTensor()(img), pos #t_transforms.ToTensor()(pos) #pos
...
class Scale(object):
	...
	def __call__(self, img, pos, t_type='pos'):
		...
		#         pos[i] = pos[i] * scale
		if pos is None:
			return img.resize((ow, oh),Image.BILINEAR), None
		return img.resize((ow, oh),Image.BILINEAR), pos.resize((ow, oh),Image.ANTIALIAS) #pos ### change to bilinear
...
class CenterCrop(object):
	...
	def __call__(self, img, pos, t_type='pos'):
		...
		return img.crop((x1, y1, x1 + tw, y1 + th)), pos.crop((x1, y1, x1 + tw, y1 + th)) #pos
